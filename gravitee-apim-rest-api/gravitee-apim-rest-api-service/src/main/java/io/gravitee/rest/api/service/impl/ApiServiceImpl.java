...
public class ApiServiceImpl extends AbstractService implements ApiService {
    ...
    public Map<String, Long> countPublishedByUserGroupedByCategories(String userId) {
        ...
        Set<String> categoryIds = extractCategoryIds(apis);
        Map<String, Long> groups = new HashMap<>();
        for (String id : categoryIds) {
            groups.put(id, categoryService.getTotalApisByCategoryId(apis, id));
        }
        return groups;
    }

    private Set<String> extractCategoryIds(Set<Api> apis) {
        return apis.stream().map(Api::getCategories).filter(Objects::nonNull).flatMap(Set::stream).collect(Collectors.toSet());
    }
}
