    ...
    private Set<Policy> policies() {
        ...
        if (getPaths() != null) {
            getPaths()
                ...
                .forEach(
                    path ->
                        policies.addAll(path.getRules().stream().filter(Rule::isEnabled).map(Rule::getPolicy).collect(Collectors.toSet()))
                );
        }
                    ...
                    if (plan.getPaths() != null) {
                        plan
                            ...
                            .forEach(
                                path ->
                                    policies.addAll(
                                        path.getRules().stream().filter(Rule::isEnabled).map(Rule::getPolicy).collect(Collectors.toSet())
                                    )
                            );
                    }
